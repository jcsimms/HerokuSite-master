...
- **Pricing Estimator** – Landing page lists **Saved estimations** (like scratchpads). **Create new estimation** opens a **3×3** grid after an interstitial. Each option opens a **question panel** (rows, times per day, users) the same width as the grid; **Add to estimate** returns to the menu. The right-hand **Estimate Workbook** has tabs **Estimation Agent** (Salesforce-style **guided / forced chat** mock with disabled composer), **Estimate Details** (captured answers), and **Deal Notebook** (placeholder). **Save Your Estimate** persists the record (including option answers). On the create screen, **only the menu grid or the Q&A block scrolls** (tabs and page header stay put); the workbook area scrolls as needed.
